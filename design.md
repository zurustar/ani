...
1.  **CLI Parser (`main` pkg)**:
    -   Parses command line flags (`-i`, `-o`, `-duration`, `-width`, `-delay`, `-bg`).
    -   `-bg` flag: strict `#RRGGBB` format only (e.g., `#FF0000`). No short HEX or color names.
...
3.  **Frame Renderer (`renderer` pkg)**:
    ...
    -   Creates a context of size `width x input_height`.
    -   Sets the background color (transparent by default, or user-specified).
...
-   **Transparency/Background**: 
    -   Default: Transparent background logic.
    -   Custom: If a `-bg` color is provided, the background is filled with that color instead of being transparent.
-   **Palette**: 
    -   We generate a dynamic palette based on the unique colors in the input image.
    -   If a background color is specified, it is added to the palette.
